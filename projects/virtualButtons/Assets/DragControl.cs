...
/**
 * Clase de ejemplo para gestionar deslizamientos por la pantalla de un movil.
 * 
 */
public class DragControl : MonoBehaviour
{
	...
	void Update ()
	{
		int vertical = 0;     //Used to store the horizontal move direction.
		int horizontal = 0;       //Used to store the vertical move direction.
		if (Input.touchCount > 0) {
			...
			//If the touch phase is not Began, and instead is equal to Ended and the x of touchOrigin is greater or equal to zero:
				...
				if (vertical != 0 || horizontal != 0) {
					onDragListener.onDrag (horizontal, vertical);
				}
				touchOrigin = myTouch.position;
			}
		}
	...
	{
		void onDrag (float horiontal, float vertical);
	}
